...
class MotionTensorboardRecorder:
    def __init__(self,
        ...
        if record_config:
            with WithoutKey('eglctx', cfg):
                if 'debug' not in cfg.exp_name:
                    cfg_file = join(record_dir, f'{cfg.exp_name}_{datetime.now().strftime("%Y%m%d_%H%M")}.yaml')
                else:
                    cfg_file = join(record_dir, f'{cfg.exp_name}_debug.yaml')
    ...
    def record(self,
        ...
        if not hasattr(self, 'writer'):  # lazy summary writer construciton to avoid creating empty log files
            ...
            if 'debug' not in cfg.exp_name:
                ...
                self.writer = SummaryWriter(log_dir=self.record_dir, filename_suffix='debug')
...
class MotionWandBRecoder:
    def __init__(self,
        ...
        if 'debug' not in cfg.exp_name and log_wandb:
            wandb.init(
                project=proj_name,
                name=cfg.exp_name,
                config=cfg,
    ...
    def record(self,
        ...
        format_scalar_stats = dict()
        for k, v in scalar_stats.items():
            v = v.median if isinstance(v, SmoothedValue) else v
            format_scalar_stats[pattern.format(k)] = v
        
        if self.log_wandb:
            wandb.log(format_scalar_stats, step=step)